...
FIXTURE_PATH = (
    Path(__file__).resolve().parent / "fixtures" / "consolidated_screening_list_lite_sample.json"
)


def test_consolidated_screening_list_lite_promotes_compliance_summary_only() -> None:
    connector = ConsolidatedScreeningListLiteConnector(
    ...
    assert promoted[0]["list_type"] == "Consolidated Screening List"
...
def test_consolidated_screening_list_lite_live_mode_is_disabled() -> None:
    connector = ConsolidatedScreeningListLiteConnector(config=ConnectorConfig(mode="live"))

    result = connector.fetch({"name": "Example"})

    assert result.status == "unavailable"
    assert "consolidated_screening_list_lite_live_fetch_not_implemented" in result.warnings
