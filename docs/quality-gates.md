...
- Browser smoke in real API mode verifies `/api/v1/sources`, `/api/v1/lineage`, System Health source registry, SemiRisk fixture graph readiness, Entity Risk 360 fixture score rendering, forward stress, reverse stress, intervention optimization, investigation report export, entity resolution, evidence lineage, and entity search. Optional full smoke can be enabled with `SUPPLY_RISK_FULL_SMOKE=1`; mock smoke is not part of the required CI gate unless the workflow explicitly adds a separate mock-mode job.
