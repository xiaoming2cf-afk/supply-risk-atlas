    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        routes = {
            ...
            "/api/v1/semiconductor/entities": lambda: main.route_semiconductor_entity_profiles(
                ...
                geo_id=_first(query.get("geo_id")),
                source_id=_first(query.get("source_id")),
                risk_tag=_first(query.get("risk_tag")),
                limit=_int_or_default(_first(query.get("limit")), 100),
                request_id=request_id,
            ),
            ...
            "/api/v1/semiconductor/relationships": lambda: main.route_semiconductor_relationships(
                relationship_class=_first(query.get("relationship_class")),
                relationship_type=_first(query.get("relationship_type")),
                edge_type=_first(query.get("edge_type")),
                source_id=_first(query.get("source_id")),
                target_id=_first(query.get("target_id")),
                layer_id=_first(query.get("layer_id")),
                stage=_first(query.get("stage")),
                source_family=_first(query.get("source_family")),
                limit=_int_or_default(_first(query.get("limit")), 100),
                request_id=request_id,
            ),
            "/api/v1/semiconductor/source-coverage": lambda: main.route_semiconductor_source_coverage(
                stage=_first(query.get("stage")),
                source_family=_first(query.get("source_family")),
                relationship_class=_first(query.get("relationship_class")),
                limit=_int_or_default(_first(query.get("limit")), 100),
